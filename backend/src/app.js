...
// Removed unused contract variable
...
app.post('/submit-vote', async (c) => {
  ...
  if (!voter.wallet_address) return c.json({ error: 'Voter wallet address not found' }, 400)

  const { data: kandidat, error: kandidatErr } = await supabase.from('kandidat').select('id').eq('id', kandidat_id).single()
  if (kandidatErr || !kandidat) return c.json({ error: 'Candidate not found' }, 404)

  const { data: existingVote } = await supabase.from('votes').select('id').eq('voter_id', voter_id).maybeSingle()
  if (existingVote) return c.json({ error: 'Voter has already voted' }, 400)
